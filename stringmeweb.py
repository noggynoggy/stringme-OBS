...
by = "von" # the word displayed between Song and artist (default is German) (set it to 'by' if you want it in English)
on = "auf" # the word displayed if the browser is on a known page (default is German) (set it to 'on' if you want it in English)
...
def activefunc():
    ...
    match endstring:   
        ...
        case "Visual Studio Code":
            ...
            extension = re.sub(r'(.*)\.(.*) - Visual Studio Code',r'\2', stractive) #.. py
            match extension:        
                    case "py":
                        extension = "  " #..    
                    case "html":
    ...
    while(checkIfProcessRunning("obs64.exe")):
        ...
        for x in range(3):                      # ↓
            ...
            for y in range(10):                 # these for-loops + the time.sleep(1) determine how fast the program runs. If you change it, dont forget to change the miliseconds in stringme_tamplate.htm in the <script> tags (default is 1000} 
                ...
                f = open("C:/Users/NOGGYlocal/.intercept-ion/stringme/stringme_web/stringme_tamplate.htm", "r", encoding="utf-8")
                fstring = f.read()
                f.close()
        
                f = open("C:/Users/NOGGYlocal/.intercept-ion/stringme/stringme_web/stringme.htm", "w", encoding="utf-8")
